...
2. Close any remaining candidate-level metamorphic, repeated-determinism, clean-environment, and chaos/fault obligations for the still-eligible LEAN path, using explicit receipts rather than inferring closure from unrelated CI.
3. Preserve the green LEAN process-fault receipt, timestamped TRACE classifier, mutation thresholds, and Nautilus negative/ineligibility/path-exhaustion evidence.
